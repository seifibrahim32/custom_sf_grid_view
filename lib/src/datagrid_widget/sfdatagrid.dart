...
class SfDataGrid extends StatefulWidget {
  ...
  /// Initialize the dataGrid UI which helps to configure color and some other
  /// properties.
  ///
...
class SfDataGridState extends State<SfDataGrid>
  ...
  late DataGridConfiguration dataGridConfiguration;
  ...
  void initState() {
    ...
    dataGridConfiguration.gridPaint = Paint();
  ...
  void _onDataGridTextDirectionChanged(TextDirection? newTextDirection) {
    ...
    dataGridConfiguration.textDirection = newTextDirection;
  ...
  void _onDataGridThemeDataChanged(
    ...
    if (_dataGridThemeData == newThemeData &&
        dataGridConfiguration.colorScheme == newColorScheme) {
      return;
    }
    ...
    if (canRefreshView) {
      _dataGridThemeHelper = DataGridThemeHelper(
          _dataGridThemeData, dataGridConfiguration.colorScheme);
      dataGridConfiguration.dataGridThemeHelper = _dataGridThemeHelper;
      _updateDecoration();
  ...
  void _onDataGridTextScaleFactorChanged(double textScaleFactor) {
    if (textScaleFactor == dataGridConfiguration.textScaleFactor) {
      return;
    }

    dataGridConfiguration.textScaleFactor = textScaleFactor;
    dataGridConfiguration.headerRowHeight = widget.headerRowHeight.isNaN
        ? (dataGridConfiguration.textScaleFactor > 1.0)
            ? _headerRowHeight * dataGridConfiguration.textScaleFactor
            : _headerRowHeight
        : widget.headerRowHeight;
    dataGridConfiguration.rowHeight = widget.rowHeight.isNaN
        ? (dataGridConfiguration.textScaleFactor > 1.0)
            ? _rowHeight * dataGridConfiguration.textScaleFactor
            : _rowHeight
        : widget.rowHeight;
    ...
    if (_container.isGridLoaded) {
      if (dataGridConfiguration.columnWidthMode != ColumnWidthMode.none) {
        resetAutoCalculation(dataGridConfiguration.columnSizer);
        if (!_container.isDirty) {
          // Refreshes the autofit columns only if don't have explicit width.
          refreshColumnSizer(dataGridConfiguration.columnSizer, 0.0);
        }
      }
      // Refresh header rows height.
      _updateHeaderRowHeight();
      // Refresh data rows height.
      _container.rowHeights.defaultLineSize = dataGridConfiguration.rowHeight;
      if (!_container.isDirty) {
  ...
  void _updateHeaderRowHeight() {
    ...
    if (_container.rowCount > 0) {
      for (int i = 0; i <= headerIndex; i++) {
        _container.rowHeights[i] = dataGridConfiguration.headerRowHeight;
      }
    }
  ...
  void _setUp() {
    ...
    //DataGrid Controller
    _controller = dataGridConfiguration.controller =
    ...
    if (widget.verticalScrollController != null) {
      dataGridConfiguration.disposeVerticalScrollController = false;
    }
    if (widget.horizontalScrollController != null) {
      dataGridConfiguration.disposeHorizontalScrollController = false;
    }
    ...
    dataGridConfiguration.columnSizer = widget.columnSizer ?? ColumnSizer();
    setStateDetailsInColumnSizer(
        dataGridConfiguration.columnSizer, _dataGridStateDetails!);
    ...
    _rowSelectionManager = dataGridConfiguration.rowSelectionManager =
  ...
  void _updateVisualDensity(VisualDensity visualDensity) {
    if (dataGridConfiguration.visualDensity == visualDensity) {
      return;
    }
  ...
  void _initializeDataGridDataSource() {
    ...
    // To fix this issue, we need to reset the source and columns in the dataGridConfiguration
    // before applying the filter when the source is changed at runtime.
    dataGridConfiguration.source = _source!;

    if (_columns != widget.columns) {
      dataGridConfiguration.columns = widget.columns;
    }
  ...
  void _initializeProperties() {
    ...
    if (_columns!.isNotEmpty) {
      ...
      _addCheckboxColumn(dataGridConfiguration);
    }
  }
  ...
  Future<void> _processUpdateDataSource() async {
    if (dataGridConfiguration.source._suspendDataPagerUpdate) {
      return;
    }
    ...
    if (widget.selectionMode != SelectionMode.none) {
      selection_manager.removeUnWantedDataGridRows(dataGridConfiguration);
    }
    if (widget.selectionMode != SelectionMode.none &&
        widget.navigationMode == GridNavigationMode.cell &&
        _rowSelectionManager != null) {
      selection_manager.onRowColumnChanged(
          dataGridConfiguration, widget.source._effectiveRows.length, -1);
    }
    ...
    if (!listEquals<GridColumn>(_columns, widget.columns)) {
      if (widget.selectionMode != SelectionMode.none &&
          ...
          _rowSelectionManager != null) {
        selection_manager.onRowColumnChanged(
            dataGridConfiguration, -1, widget.columns.length);
      }
      ...
      if (dataGridConfiguration.showCheckboxColumn &&
        ...
        if (dataGridConfiguration.controller.selectedRows.isEmpty) {
    ...
    setState(() {});
    if (dataGridConfiguration.source.shouldRecalculateColumnWidths()) {
      resetAutoCalculation(dataGridConfiguration.columnSizer);
    }
  ...
  void _refreshScrollOffsets([bool canRefreshHorizontalOffset = false]) {
    ...
    if (dataGridConfiguration.verticalScrollController != null &&
        ...
        dataGridConfiguration.verticalScrollController!.offset == 0 &&
        dataGridConfiguration.container.verticalOffset > 0) {
    ...
    if (canRefreshHorizontalOffset) {
      if (dataGridConfiguration.horizontalScrollController != null &&
          ...
          dataGridConfiguration.horizontalScrollController!.offset == 0) {
        final double maxScrollExtent = dataGridConfiguration
            .horizontalScrollController!.position.maxScrollExtent;
        if (dataGridConfiguration.textDirection == TextDirection.ltr &&
            dataGridConfiguration.container.horizontalOffset > 0.0) {
          dataGridConfiguration.container
            ..horizontalOffset = 0
            ..horizontalScrollBar.value = 0;
        } else if (dataGridConfiguration.textDirection == TextDirection.rtl &&
  ...
  void _processEditing() {
    ...
    final DataRowBase? dataRow = dataGridConfiguration.rowGenerator.items
  ...
  void _resetColumn({bool clearEditing = true}) {
    if (_columns != null) {
      _columns!
        ..clear()
        ..addAll(widget.columns);
      dataGridConfiguration.columns = _columns!;
      if (_columns!.isNotEmpty) {
      ...
      if (rowColumnIndex != null) {
        ...
        setState(() {
          ...
          if (recalculateRowHeight) {
            dataGridConfiguration.container.rowHeightManager
                .setDirty(rowIndex);
            dataGridConfiguration.container
    ...
    if (propertyName == 'columnDragAndDrop') {
      ...
      if (dataGridConfiguration
        ...
        if (dataGridConfiguration.columnWidthMode ==
            ...
            dataGridConfiguration.columns.firstWhereOrNull(
                    (GridColumn element) =>
  ...
  void _updateDataGridStateDetails() {
    dataGridConfiguration
      ...
      ..rowGenerator = _rowGenerator
      ..visualDensity = dataGridConfiguration.visualDensity
      ...
      ..rowHeight = (widget.rowHeight.isNaN
          ? dataGridConfiguration.rowHeight.isNaN
              ? _rowHeight
              : dataGridConfiguration.rowHeight
          : widget.rowHeight)
      ..headerRowHeight = (widget.headerRowHeight.isNaN
          ? dataGridConfiguration.headerRowHeight.isNaN
              ? _headerRowHeight
    ...
    if (widget.allowPullToRefresh) {
      dataGridConfiguration.refreshIndicatorKey ??=
  ...
  Future<void> _updateProperties(SfDataGrid oldWidget) async {
    ...
    if (!oldWidget.allowColumnsResizing && widget.allowColumnsResizing) {
      dataGridConfiguration.columnResizeController.setHitTestPrecision();
    }
    ...
    if (isSourceChanged ||
      ...
      if (isSortingChanged || isMultiColumnSortingChanged) {
        if (!widget.allowSorting) {
          ...
          while (dataGridConfiguration.source.sortedColumns.length > 1) {
            dataGridConfiguration.source.sortedColumns.removeAt(0);
          }
          dataGridConfiguration.source._updateDataSource();
        }
      }
      ...
      if (oldWidget.gridLinesVisibility != widget.gridLinesVisibility ||
          ...
          isEditingChanged) {
        // Need to endEdit before refreshing
        if (isEditingChanged && dataGridConfiguration.currentCell.isEditing) {
          await dataGridConfiguration.currentCell
              .onCellSubmit(dataGridConfiguration, canRefresh: false);
        }
        _initializeProperties();
  ...
  void _onStackedHeaderRowsPropertyChanged(
    ...
    if ((oldWidget.stackedHeaderRows.isNotEmpty &&
      ...
      if (dataGridConfiguration.verticalScrollController!.hasClients &&
          dataGridConfiguration.container.verticalOffset > 0) {
        dataGridConfiguration.container.verticalOffset = 0;
        dataGridConfiguration.container.verticalScrollBar.value = 0;
      }
      if (dataGridConfiguration.horizontalScrollController!.hasClients &&
          dataGridConfiguration.container.horizontalOffset > 0) {
        dataGridConfiguration.container.horizontalOffset = 0;
        dataGridConfiguration.container.horizontalScrollBar.value = 0;
      }
    }
  }
  ...
  void _updateDecoration() {
    ...
    dataGridConfiguration.boxPainter = decoration.createBoxPainter();
  }
  ...
  Future<void> refresh([bool showRefreshIndicator = true]) async {
    if (dataGridConfiguration.allowPullToRefresh &&
        dataGridConfiguration.refreshIndicatorKey != null) {
      if (showRefreshIndicator) {
        await dataGridConfiguration.refreshIndicatorKey!.currentState?.show();
      } else {
        await dataGridConfiguration.source.handleRefresh();
      }
    }
  }
  ...
  void didChangeDependencies() {
    ...
    dataGridConfiguration.isDesktop = kIsWeb ||
    ...
    dataGridConfiguration.defaultColumnWidth = widget.defaultColumnWidth.isNaN
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      ...
      final double measuredHeight = dataGridConfiguration.viewHeight =
          constraints.maxHeight.isInfinite ? _minHeight : constraints.maxHeight;
      double measuredWidth = dataGridConfiguration.viewWidth =
      ...
      if (dataGridConfiguration.shrinkWrapColumns) {
        measuredWidth = dataGridConfiguration.viewWidth =
      ...
      if (!_container.isGridLoaded) {
        ...
        updateColumnSizerLoadedInitiallyFlag(
            dataGridConfiguration.columnSizer, true);
      }
  ...
  void dispose() {
    ...
    dataGridConfiguration.dataGridFilterHelper!.checkboxFilterHelper
      ..textController.dispose()
      ..searchboxFocusNode.dispose();
    dataGridConfiguration.dataGridFilterHelper!.advancedFilterHelper
    ...
    dataGridConfiguration.dataGridFilterHelper!
        .resetColumnProperties(dataGridConfiguration);
    super.dispose();
  }
}
